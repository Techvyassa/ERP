...
class GrantAllModulePermissions extends Command
{
    protected $signature = 'tenant:grant-all-permissions 
                            {tenant_db : The tenant database name}
                            {role_code=ADMIN : The role code to grant permissions to (default: ADMIN)}';

    protected $description = 'Grant all module permissions to a role (INVENTORY, PO, VENDOR, etc.)';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->info("\nGranting permissions for modules:");
            ...
            foreach ($modules as $moduleCode) {
                ...
                $this->info("  ✓ {$moduleCode}");
            }

            // Clear permission cache
            \Illuminate\Support\Facades\Cache::flush();
            $this->info("\n✓ All permissions granted to {$role->role_name}");
