...
<head>
    ...
    <title>@yield('title', 'Maintenance Portal') - {{ $organization->org_name }}</title>
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                <div class="flex items-center space-x-3" x-show="sidebarOpen">
                    <div class="w-10 h-10 bg-maintenance rounded-lg flex items-center justify-center flex-shrink-0">
                        <span class="material-symbols-outlined text-white text-xl">build</span>
                    </div>
                    <div class="overflow-hidden">
                        <h2 class="text-sm font-semibold text-gray-900 truncate">{{ $organization->org_name }}</h2>
                        <p class="text-xs text-gray-500 truncate">Maintenance Portal</p>
                    </div>
                </div>
                <div x-show="!sidebarOpen" class="w-10 h-10 bg-maintenance rounded-lg flex items-center justify-center mx-auto">
                    <span class="material-symbols-outlined text-white text-xl">build</span>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto p-4">
                <ul class="space-y-2">
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/maintenance/dashboard") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.maintenance.dashboard') ? 'bg-amber-50 text-maintenance font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
                    ...
                    <li x-show="sidebarOpen" class="px-3 py-2">
                        <span class="text-xs font-semibold text-gray-400 uppercase">Maintenance</span>
                    </li>
                    ...
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/maintenance/schedule") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.maintenance.schedule*') ? 'bg-amber-50 text-maintenance font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
                            <span class="material-symbols-outlined text-lg w-5">calendar_month</span>
                            <span x-show="sidebarOpen" class="font-medium">PM Schedule</span>
                        </a>
                    </li>

                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/maintenance/spare-parts") }}"
                    ...
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/users") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.users.*') ? 'bg-amber-50 text-maintenance font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
                    ...
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/departments") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.departments.*') ? 'bg-amber-50 text-maintenance font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
            ...
            <div class="border-t border-gray-200 p-4">
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center space-x-3 w-full px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors">
                        <div class="w-10 h-10 bg-maintenance rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <div x-show="open" @click.away="open = false" x-cloak
                        ...
                        <form action="{{ route('tenant.logout', $organization->org_slug) }}" method="POST">
                            @csrf
                            <input type="hidden" name="redirect_to" value="/org/{{ $organization->org_slug }}/maintenance/login">
        ...
        <header class="bg-white border-b border-gray-200 sticky top-0 z-30">
            <div class="flex items-center justify-between px-6 py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">@yield('page-title', 'Maintenance Portal')</h1>
                </div>
                <div class="flex items-center space-x-4">
