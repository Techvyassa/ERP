...
<body class="bg-gray-50" x-data="{ sidebarOpen: true, user: {} }"
    x-init="user = JSON.parse(localStorage.getItem('user') || '{}');">
    <!-- Sidebar -->
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                ...
                <div x-show="!sidebarOpen"
                    class="w-10 h-10 bg-production rounded-lg flex items-center justify-center mx-auto">
                    <span class="material-symbols-outlined text-white text-xl">precision_manufacturing</span>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto p-4">
                <ul class="space-y-1">
                    ...
                    <li x-show="sidebarOpen" class="px-3 pt-2 pb-1">
                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Production
                            Flow</span>
                    </li>
                    ...
                    {{-- Step 3: Material Receiving — confirm materials arrived --}}
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/production/receiving") }}"
                            ...
                            <span x-show="sidebarOpen" class="font-medium">Material Receiving</span>
                        </a>
                    </li>
            ...
            <div class="border-t border-gray-200 p-4">
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        class="flex items-center space-x-3 w-full px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors">
                        <div
                            class="w-10 h-10 bg-production rounded-full flex items-center justify-center flex-shrink-0">
                            <span class="text-white font-semibold text-sm"
                                x-text="user.first_name && user.last_name ? (user.first_name.charAt(0) + user.last_name.charAt(0)).toUpperCase() : 'U'"></span>
                        </div>
                        <div x-show="sidebarOpen" class="flex-1 text-left overflow-hidden">
                            <p class="text-sm font-medium text-gray-900 truncate"
                                x-text="user.first_name && user.last_name ? user.first_name + ' ' + user.last_name : 'User'">
                            </p>
                    ...
                    <div x-show="open" @click.away="open = false" x-cloak
                        ...
                        <form action="{{ route('tenant.logout', $organization->org_slug) }}" method="POST">
                            ...
                            <input type="hidden" name="redirect_to"
                                value="/org/{{ $organization->org_slug }}/production/login">
                            <button type="submit"
