...
<head>
    ...
    <title>@yield('title', 'Sales Portal') - {{ $organization->org_name }}</title>
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                <div class="flex items-center space-x-3" x-show="sidebarOpen">
                    <div class="w-10 h-10 bg-sales rounded-lg flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="text-xs text-gray-500 truncate">Sales Portal</p>
                    </div>
                </div>
                <div x-show="!sidebarOpen" class="w-10 h-10 bg-sales rounded-lg flex items-center justify-center mx-auto">
                    <span class="material-symbols-outlined text-white text-xl">storefront</span>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto p-4">
                <ul class="space-y-2">
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/sales/dashboard") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.sales.dashboard') ? 'bg-emerald-50 text-sales font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
                    ...
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/users") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.users.*') ? 'bg-emerald-50 text-sales font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
                    ...
                    <li>
                        <a href="{{ url("/org/{$organization->org_slug}/departments") }}"
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.departments.*') ? 'bg-emerald-50 text-sales font-semibold' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
            ...
            <div class="border-t border-gray-200 p-4">
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center space-x-3 w-full px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors">
                        <div class="w-10 h-10 bg-sales rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <div x-show="open" @click.away="open = false" x-cloak
                        ...
                        <form action="{{ route('tenant.logout', $organization->org_slug) }}" method="POST">
                            @csrf
                            <input type="hidden" name="redirect_to" value="/org/{{ $organization->org_slug }}/sales/login">
        ...
        <header class="bg-white border-b border-gray-200 sticky top-0 z-30">
            <div class="flex items-center justify-between px-6 py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">@yield('page-title', 'Sales Portal')</h1>
                </div>
                <div class="flex items-center space-x-4">
