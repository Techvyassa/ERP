...
@section('content')
<div x-data="salesOrdersPage()" x-init="init()">

    <!-- Header -->
    <div class="bg-gradient-to-r from-indigo-700 to-indigo-900 rounded-xl p-6 mb-6 text-white shadow-lg">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold mb-1">Sales Order Creation</h2>
                    <p class="text-white/80 text-sm">Sales team creates orders with customer details, products, quantity and required delivery date. Triggers outward flow and stock availability check.</p>
                </div>
            </div>
            <button @click="openCreateModal()"
                class="flex items-center gap-2 bg-white text-indigo-700 hover:bg-indigo-50 font-semibold px-5 py-2.5 rounded-lg transition-colors text-sm">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center hover:shadow-md transition-shadow">
            <p class="text-3xl font-bold text-indigo-700" x-text="soStats.total_open">0</p>
            <p class="text-xs text-gray-500 mt-1">Open Orders</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center hover:shadow-md transition-shadow">
            <p class="text-3xl font-bold text-green-600" x-text="soStats.stock_available">0</p>
            <p class="text-xs text-gray-500 mt-1">Stock Available</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center hover:shadow-md transition-shadow">
            <p class="text-3xl font-bold text-amber-600" x-text="soStats.stock_partial">0</p>
            <p class="text-xs text-gray-500 mt-1">Partial Stock</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center hover:shadow-md transition-shadow">
            <p class="text-3xl font-bold text-red-600" x-text="soStats.due_today">0</p>
            <p class="text-xs text-gray-500 mt-1">Due Today</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center hover:shadow-md transition-shadow">
            <p class="text-3xl font-bold text-purple-600" x-text="soStats.pending_dispatch">0</p>
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="flex items-center justify-between p-5 border-b border-gray-100">
            <h3 class="font-bold text-gray-900">All Sales Orders</h3>
            <div class="flex items-center gap-3">
                <input x-model="soSearch" @input.debounce.400ms="loadSalesOrders()"
                    type="text" placeholder="Search SO number..."
                    class="border border-gray-300 rounded-lg px-3 py-2 text-sm w-52 focus:outline-none focus:ring-2 focus:ring-indigo-300">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-600 text-xs uppercase">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-center font-semibold">Stock Status</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <template x-if="!loading && salesOrders.length === 0">
                    ...
                    <template x-for="so in salesOrders" :key="so.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-5 py-3 font-semibold text-indigo-700" x-text="so.so_number"></td>
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100 text-sm text-gray-500">
            <span x-text="'Showing ' + salesOrders.length + ' of ' + pagination.total + ' orders'"></span>
    ...
    <!-- Create Modal (same as dashboard) -->
...
function salesOrdersPage() {
    return {
        salesOrders: [], loading: false, soSearch: '', soStatusFilter: '',
        soStats: { total_open: 0, due_today: 0, stock_available: 0, stock_partial: 0, pending_dispatch: 0 },
        pagination: { current_page: 1, last_page: 1, total: 0 },
        showCreateModal: false, customers: [], products: [], uoms: [],
        form: { customer_id: '', required_delivery_date: '', payment_terms: 'NET30', remarks: '', line_items: [] },
        formError: '', formSubmitting: false,

        init() { this.loadSOStats(); this.loadSalesOrders(); },

        async loadSOStats() {
            const token = localStorage.getItem('auth_token');
            const res = await fetch('/api/v1/sales-orders/dashboard-stats', { headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' } });
        ...
        async loadSalesOrders() {
            ...
            const res = await fetch('/api/v1/sales-orders?' + params, { headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' } });
            const json = await res.json();
            if (json.success) {
                this.salesOrders = json.data.data;
                this.pagination = { current_page: json.data.current_page, last_page: json.data.last_page, total: json.data.total };
            }
            this.loading = false;
        },

        prevPage() { if (this.pagination.current_page > 1) { this.pagination.current_page--; this.loadSalesOrders(); } },
        nextPage() { if (this.pagination.current_page < this.pagination.last_page) { this.pagination.current_page++; this.loadSalesOrders(); } },

        async openCreateModal() {
            this.form = { customer_id: '', required_delivery_date: '', payment_terms: 'NET30', remarks: '', line_items: [this.emptyLine()] };
            this.formError = ''; this.formSubmitting = false;
            const token = localStorage.getItem('auth_token');
            const headers = { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' };
            const [cRes, pRes, uRes] = await Promise.all([fetch('/api/v1/customers?active_only=1', { headers }), fetch('/api/v1/products', { headers }), fetch('/api/v1/uoms', { headers })]);
        ...
        async submitSalesOrder() {
            this.formError = '';
            if (!this.form.customer_id || !this.form.required_delivery_date) { this.formError = 'Customer and delivery date are required.'; return; }
            if (this.form.line_items.some(l => !l.product_id || !l.qty || !l.uom_id)) { this.formError = 'All line items must have product, quantity and UOM.'; return; }
            this.formSubmitting = true;
            const token = localStorage.getItem('auth_token');
            const res = await fetch('/api/v1/sales-orders', { method: 'POST', headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(this.form) });
            const json = await res.json();
            if (json.success) { this.showCreateModal = false; await this.loadSalesOrders(); await this.loadSOStats(); }
            else { this.formError = json.message || Object.values(json.errors ?? {}).flat().join(', '); }
