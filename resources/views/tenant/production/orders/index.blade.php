                    ...
                    <template x-for="order in orders" :key="order.id">
                        <tr class="group hover:bg-orange-50/30 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                ...
                                <span x-show="order.is_request"
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-black rounded-full ring-1 ring-inset uppercase tracking-wide bg-amber-50 text-amber-700 ring-amber-200">
                                    READY
                                </span>
                            </td>
                                ...
                                <div class="inline-flex items-center gap-1">
                                    ...
                                    <template x-if="order.status === 'DRAFT' || order.status === 'READY'">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <template x-if="!order.is_request && order.mir_status === 'FULLY_ISSUED'">
                                            ...
                                            <template x-if="order.mir_status === 'CLOSED'">
                                                <button @click="order.is_request ? startFromRequest(order) : startOrder(order)"
...
<script>
    function productionOrders(orgSlug) {
        return {
            ...
            async submitOrder() {
                ...
                try {
                    ...
                    window.dispatchEvent(new CustomEvent('notify', {
                        detail: {
                            message: 'Production order created. MIR sent to Store for approval.',
                            type: 'success'
                        }
                    }));
                } catch (e) {
            ...
            async startFromRequest(request) {
                this.confirm(
                    'Start Production',
                    `Convert this request ${request.order_no} to a production order and start manufacturing?`,
                    async () => {
                            try {
                                // 1. Convert to order
                                const convRes = await this._fetch(`/api/v1/production-requests/${request.id}/convert-to-order`, {
                                    method: 'POST'
                                });
                                const convData = await convRes.json();
                                if (!convRes.ok || !convData.success) throw new Error(convData.message || 'Failed to convert request');

                                const newOrderId = convData.data.order.id;

                                // 2. Start the new order
                                const startRes = await this._fetch(`/api/v1/production-orders/${newOrderId}/start`, {
                                    method: 'POST'
                                });
                                const startData = await startRes.json();
                                if (!startRes.ok || !startData.success) throw new Error(startData.message || 'Failed to start production');
