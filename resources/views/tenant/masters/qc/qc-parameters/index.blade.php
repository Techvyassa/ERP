        ...
        <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-6 mb-6">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-sm text-gray-500">Maintain clean, reusable inspection parameters for each material.
                    </p>
                </div>
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center">
                ...
                <select x-model="filters.material_id" @change="loadParameters()"
                    ...
                    <template x-for="material in materials" :key="material.id">
                        <option :value="material.id" x-text="`${material.material_code} - ${material.material_name}`">
                        </option>
                    </template>
                </select>
                        ...
                        <template x-if="!loading && items.length === 0">
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 mt-1">Create your first parameter to start building QC
                                        standards.</p>
                                    <button @click="openCreateModal()"
                                        class="mt-4 px-4 py-2 bg-sky-600 text-white rounded-xl hover:bg-sky-700 transition-colors">
                            ...
                            <tr class="hover:bg-gray-50/70 transition-colors">
                                <td class="px-6 py-4 align-top">
                                    ...
                                    <div class="mt-2 text-xs text-gray-500"
                                        x-text="item.parameter_category || 'No category'"></div>
                                </td>
                                <td class="px-6 py-4 align-top text-sm text-gray-700">
                                    <div class="font-medium text-gray-900" x-text="item.material?.material_name || '-'">
                                    </div>
                                    <div class="text-xs text-gray-500 mt-1" x-text="item.material?.material_code || '-'">
                                    </div>
                                    <div class="text-xs text-gray-500 mt-2"
                                        x-text="item.test_type?.type_name || 'Unassigned'"></div>
                                </td>
                                <td class="px-6 py-4 align-top text-sm text-gray-700">
                                    <div class="font-medium text-gray-900" x-text="formatTolerance(item)"></div>
                                    <div class="text-xs text-gray-500 mt-1" x-text="item.test_method || 'No test method'">
                                    </div>
                                    <div class="text-xs text-gray-400 mt-1">Order: <span
                                            x-text="item.display_order ?? 0"></span></div>
                                </td>
                                <td class="px-6 py-4 align-top">
                                    ...
                                    <div class="text-xs text-gray-500"
                                        x-text="item.data_type + ' / ' + item.tolerance_type"></div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-3">
                                        <button @click="openEditModal(item)"
                                            class="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-blue-600 hover:bg-blue-50 hover:text-blue-800">
                                            <i class="fas fa-edit"></i>
                                            <span class="hidden sm:inline">Edit</span>
                                        </button>
                                        <button @click="deactivateParameter(item)"
                                            class="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50 hover:text-red-800">
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-gray-200">
                    <div>
                        <h3 class="text-xl font-semibold text-gray-900"
                            x-text="editId ? 'Edit QC Parameter' : 'Create QC Parameter'"></h3>
                        <p class="text-sm text-gray-500 mt-1">Enter the parameter details and define the inspection standard
                            clearly.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-2xl border border-gray-200 p-5">
                        ...
                        <div>
                            ...
                            <div x-show="form.source_type !== 'product'">
                                <select x-model="form.material_id"
                                    :required="form.source_type !== 'product'"
                                    ...
                                    <template x-for="material in materials" :key="material.id">
                                        <option :value="material.id"
                                            x-text="`${material.material_code} - ${material.material_name}`"></option>
                                    </template>
                                </select>
                                <p class="mt-1 text-xs text-gray-500">Missing material? Add it in the material master, then
                                    come back here.</p>
                            </div>
                                ...
                                <select x-model="form.product_id"
                                    ...
                                    <template x-for="product in products" :key="product.id">
                                        <option :value="product.id"
                                            x-text="`${product.product_code} - ${product.product_name}`"></option>
                                    </template>
                                </select>
                                <p class="mt-1 text-xs text-gray-500">These parameters will be used for Finished Goods QC
                                    inspections.</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select x-model="form.test_type_id" @change="syncCategoryFromTestType()"
                                ...
                                <template x-for="testType in testTypes" :key="testType.id">
                                    <option :value="testType.id" x-text="`${testType.type_code} - ${testType.type_name}`">
                                    </option>
                                </template>
                            </select>
                            <p class="mt-1 text-xs text-gray-500">Use test types to keep QC categories consistent across
                                parameters.</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-2xl border border-gray-200 p-5">
                        ...
                        <div>
                            ...
                            <select x-model="form.data_type" @change="onDataTypeChange()" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                                <option value="NUMERIC">Numeric — measured value (e.g. pH, moisture %)</option>
                                <option value="BOOLEAN">Pass / Fail — binary check (e.g. visual, leak test)</option>
                                <option value="TEXT">Text / Custom — descriptive check (e.g. colour, odour)</option>
                            </select>
                        </div>

                        <div x-show="form.data_type === 'NUMERIC'">
                            ...
                            <select x-model="form.tolerance_type" @change="onToleranceTypeChange()" required
                                ...
                                <option value="MIN_ONLY">Min Only — must be ≥ value</option>
                                <option value="MAX_ONLY">Max Only — must be ≤ value</option>
                                <option value="EXACT">Exact — must equal value</option>
                            </select>
                        </div>

                        <!-- Pass/Fail hint -->
                        <div x-show="form.data_type === 'BOOLEAN'"
                            class="flex items-start gap-2 px-3 py-2.5 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-700">
                            <span class="material-symbols-outlined text-sm mt-0.5">info</span>
                            <span>Pass / Fail uses a fixed expected value. Set <strong>Standard Value</strong> to <code>Pass</code> or <code>Fail</code>.</span>
                        </div>

                        <!-- Text/Custom hint -->
                        <div x-show="form.data_type === 'TEXT'"
                            class="flex items-start gap-2 px-3 py-2.5 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-700">
                            <span class="material-symbols-outlined text-sm mt-0.5">info</span>
                            <span>Text check uses a fixed expected description. E.g. <em>"No Bubbles"</em>, <em>"White Powder"</em>, <em>"Characteristic Odour"</em>.</span>
                        </div>
                        ...
                        <div x-show="showExactField()">
                            <label class="block text-sm font-medium text-gray-700 mb-1"
                                x-text="form.data_type === 'BOOLEAN' ? 'Expected Result' : form.data_type === 'TEXT' ? 'Expected Description' : 'Standard Value'">
                            </label>
                            <!-- Pass/Fail quick buttons -->
                            <template x-if="form.data_type === 'BOOLEAN'">
                                <div class="flex gap-2 mb-2">
                                    <button type="button" @click="form.standard_value = 'Pass'"
                                        :class="form.standard_value === 'Pass' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600'"
                                        class="flex-1 py-2 rounded-lg text-sm font-bold transition-all">
                                        ✓ Pass
                                    </button>
                                    <button type="button" @click="form.standard_value = 'Fail'"
                                        :class="form.standard_value === 'Fail' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-600'"
                                        class="flex-1 py-2 rounded-lg text-sm font-bold transition-all">
                                        ✗ Fail
                                    </button>
                                </div>
        ...
        function qcParameterData() {
            ...
            return {
                ...
                presets: [
                    ...
                    { key: 'minimum_limit', label: 'Min Limit', icon: 'fas fa-arrow-up', help: 'For purity or strength checks.', data_type: 'NUMERIC', tolerance_type: 'MIN_ONLY', parameter_category: 'CHEMICAL' },
                    { key: 'maximum_limit', label: 'Max Limit', icon: 'fas fa-arrow-down', help: 'For contamination or moisture caps.', data_type: 'NUMERIC', tolerance_type: 'MAX_ONLY', parameter_category: 'CHEMICAL' },
                    { key: 'pass_fail', label: 'Pass / Fail', icon: 'fas fa-toggle-on', help: 'Visual or binary checks (e.g. leak test, seal integrity).', data_type: 'BOOLEAN', tolerance_type: 'EXACT', parameter_category: 'VISUAL' },
                    { key: 'text_custom', label: 'Custom Text', icon: 'fas fa-align-left', help: 'Descriptive checks (e.g. colour, odour, appearance).', data_type: 'TEXT', tolerance_type: 'EXACT', parameter_category: 'PHYSICAL' }
                ],
                form: {},
                ...
                openEditModal(item) {
                    this.editId = item.id;
                    const isProduct = !!item.product_id && !item.material_id;
                    this.form = {
                        source_type: isProduct ? 'product' : 'material',
                ...
                onDataTypeChange() {
                    ...
                    if (this.form.data_type === 'BOOLEAN') {
                        ...
                        if (!this.form.standard_value || this.form.standard_value === 'true') {
                            this.form.standard_value = 'Pass';
                        }
                    }
                    if (this.form.data_type === 'TEXT') {
                        this.form.tolerance_type = 'EXACT';
                        this.form.standard_value = this.form.standard_value || '';
                    }
                },
                ...
                exactPlaceholder() {
                    if (this.form.data_type === 'BOOLEAN') return 'Pass or Fail';
                    if (this.form.data_type === 'TEXT') return 'e.g. No Bubbles, White Powder, Characteristic Odour';
                ...
                async saveParameter() {
                    this.saving = true;
                    try {
                        const isProduct = this.form.source_type === 'product';

                        const payload = {
                            // Send only the relevant ID, null for the other
                            material_id: isProduct ? null : (this.form.material_id || null),
                            product_id: isProduct ? (this.form.product_id || null) : null,
                        ...
                        if (data.success) {
                            ...
                            const errMsg = data.message || (data.error?.details ? Object.values(data.error.details).flat().join(', ') : 'Failed to save QC parameter');
