...
<div x-data="contactData()" x-init="loadData()">
    ...
    <div class="bg-white rounded-xl shadow p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input type="text" x-model="filters.search" @input="loadData" placeholder="Search by name..." 
                   class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <input type="text" x-model="filters.vendor" @input="loadData" placeholder="Filter by vendor..." 
                   class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <select x-model="filters.contact_type" @change="loadData" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-if="loading">
                        <tr><td colspan="8" class="px-6 py-12 text-center"><i class="fas fa-spinner fa-spin text-3xl text-gray-400"></i><p class="text-gray-600 mt-2">Loading contacts...</p></td></tr>
                    </template>
                    <template x-if="!loading && items.length === 0">
                        <tr><td colspan="8" class="px-6 py-12 text-center"><i class="fas fa-address-book text-6xl text-gray-300 mb-4"></i><p class="text-gray-600">No vendor contacts found.</p></td></tr>
                    </template>
                    <template x-for="item in items" :key="item.id">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-900" x-text="item.vendor_name"></td>
                            <td class="px-6 py-4 whitespace-nowrap"><span class="text-sm font-medium text-gray-900" x-text="item.contact_name"></span></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800" x-text="item.contact_type"></span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs rounded-full" :class="item.is_primary ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'" x-text="item.is_primary ? 'Yes' : 'No'"></span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
...
function contactData() {
    return {
        items: [], loading: false, filters: { search: '', vendor: '', contact_type: '' },
        async loadData() { this.loading = true; try { this.items = []; } catch (e) { alert('Failed to load contacts.'); } finally { this.loading = false; } },
        resetFilters() { this.filters = { search: '', vendor: '', contact_type: '' }; this.loadData(); },
        openCreateModal() { alert('Create contact - Coming soon'); },
        edit(item) { alert('Edit contact: ' + item.contact_name + ' - Coming soon'); },
        async deleteItem(item) { if (confirm('Delete contact: ' + item.contact_name + '?')) { alert('Delete functionality - Coming soon'); } }
    }
}
</script>
@endsection
