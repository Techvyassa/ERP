...
<head>
    ...
    <title>@yield('title', 'Tax & Financial') - {{ $organization->org_name }}</title>
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                <div class="flex items-center space-x-3" x-show="sidebarOpen">
                    ...
                    <div class="overflow-hidden">
                        <h2 class="text-sm font-semibold text-gray-900 truncate">Tax & Financial</h2>
                ...
                <ul class="space-y-2">
                    ...
                    <li>
                        <a href="{{ url($tenantType === 'subdomain' ? '/tax-dashboard' : "/org/{$organization->org_slug}/tax-dashboard") }}" 
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.tax-dashboard') ? 'bg-green-50 text-category' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
                            <span class="material-symbols-outlined text-lg w-5">dashboard</span>
                            <span x-show="sidebarOpen" class="font-medium">Tax Dashboard</span>
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="{{ url($tenantType === 'subdomain' ? '/currency' : "/org/{$organization->org_slug}/currency") }}" 
                           class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('tenant.currency.*') ? 'bg-green-50 text-category' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
            ...
            <div class="flex items-center justify-between px-6 py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-category">receipt_long</span>
                        <h1 class="text-xl font-semibold text-gray-900">@yield('page-title', 'Tax & Financial')</h1>
                    </div>
                </div>
                
                <div class="flex items-center space-x-4">
                    <span class="text-xs text-green-600 px-3 py-1 bg-green-50 rounded-full font-semibold">
                        Tax & Financial
                    </span>
                </div>
            </div>
